          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">{total} crew members total</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-full px-4 sm:px-6 lg:px-8 py-8">
        {data.length === 0 ? (
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-12 text-center">
            ...
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">No Crew Data Found</h3>
            <p className="text-gray-600 dark:text-gray-400 mb-6">Upload an Excel file to see crew member information here.</p>
            <Link
              href="/modules/crew-planning/admin"
                ...
                <table className="min-w-full text-sm">
                  <thead>
                    <tr className="bg-gradient-to-r from-slate-700 via-slate-800 to-slate-900 dark:from-slate-800 dark:via-slate-900 dark:to-black">
                      {columns.map((col, idx) => (
                        ...
                        >
                          <span className="text-xs tracking-widest font-semibold drop-shadow-md">{col}</span>
                        </th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-700/50">
                    {data.map((member, rowIdx) => (
                      <tr
                        key={member.id}
                        className={`transition-all duration-150 ${rowIdx % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-gray-50/50 dark:bg-gray-900/30'} hover:bg-blue-50 hover:dark:bg-blue-900/20 hover:shadow-md`}
                      >
                        {columns.map((col, colIdx) => (
                          <td
                            key={colIdx}
                            className="px-6 py-3 text-gray-800 dark:text-gray-100 whitespace-nowrap border-r border-gray-100 dark:border-gray-700/30 last:border-r-0 text-sm"
                          >
                            {(() => {
